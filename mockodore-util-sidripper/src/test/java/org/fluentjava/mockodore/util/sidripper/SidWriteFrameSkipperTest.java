...
public class SidWriteFrameSkipperTest {
	...
	private static void writeAllRegs(SidWriteFrameSkipper sid, UnsignedByte v) {
		// TODO handle these convenience methods in one place
		// instead of having to handle them in all impls and their tests:
		for (OscName osc : Arrays.asList(OscName.OSC_1, OscName.OSC_2,
				OscName.OSC_3)) {
			sid.ad(osc).write(v);
			sid.cr(osc).write(v);
			sid.freqHi(osc).write(v);
			sid.freqLo(osc).write(v);
			sid.pwHi(osc).write(v);
			sid.pwLo(osc).write(v);
			sid.sr(osc).write(v);
		}
		sid.fcHi().write(v);
		sid.fcLo().write(v);
		sid.modeVol().write(v);
		sid.resFilt().write(v);
		// get coverage for this too:
		sid.reg(SidRegisterAddress.RES_FILT).write(v);
	}
